...
Для сравнения подходов к управлению зависимостями было реализовано альтернативное решение на основе DI контейнера GetIt. В отличие от Inherited Widget, GetIt не требует BuildContext для доступа к зависимостям, что делает код более гибким и удобным в использовании.

=== Регистрация зависимостей

Регистрация всех зависимостей происходит в точке входа приложения — функции main():

#listing(
  file: "../lib/main.dart",
  caption: [Регистрация зависимостей в GetIt контейнере],
  from: 7,
  to: 23,
) <getit-main>

В #r(<getit-main>) показана регистрация трех основных зависимостей:

- `HabbitIdGenerator` — функция-генератор уникальных ID для привычек. Регистрируется как singleton с помощью `registerSingleton`. Использует замыкание для хранения счетчика
- `CurrentDateTimeProvider` — функция для получения текущего времени. Также регистрируется как singleton
- `HabbitsController` — главный контроллер приложения. Регистрируется с помощью `registerLazySingleton`, что означает отложенную инициализацию (объект создается только при первом обращении)

Использование type aliases (`HabbitIdGenerator` и `CurrentDateTimeProvider`) позволяет явно указывать типы зависимостей при регистрации и получении из контейнера.

=== Использование в экранах

Доступ к контроллеру через GetIt не требует BuildContext, что упрощает код и делает его более читаемым:

#listing(
  file: "../lib/features/screens/habbits_list_screen.dart",
  caption: [Использование GetIt в экране списка привычек],
  from: 8,
  to: 29,
) <getit-list-screen>

В #r(<getit-list-screen>) видно, как контроллер получается из GetIt контейнера на строке 19. Обратите внимание на использование `ListenableBuilder` для автоматического обновления UI при изменении состояния контроллера — это работает благодаря тому, что `HabbitsController` наследуется от `ChangeNotifier`.

=== Использование в форме добавления

В форме добавления привычки GetIt используется для получения контроллера без необходимости передавать его через конструктор:

#listing(
  file: "../lib/features/screens/habbit_form_screen.dart",
  caption: [Получение данных редактируемой привычки через GetIt],
  from: 59,
  to: 63,
) <getit-form-editing>
...
#listing(
  file: "../lib/features/screens/habbit_form_screen.dart",
  caption: [Отправка данных формы через контроллер из GetIt],
  from: 69,
  to: 87,
) <getit-form-submit>

В #r(<getit-form-editing>) показано получение данных редактируемой привычки. Контроллер доступен напрямую через `GetIt.I.get<HabbitsController>()` без необходимости пробрасывать зависимость через дерево виджетов.

В #r(<getit-form-submit>) демонстрируется отправка данных формы. Метод `_submitForm` вызывает либо `addHabbit`, либо `editHabbit` в зависимости от режима работы формы (создание или редактирование).

=== Преимущества GetIt перед Inherited Widget

Реализация с GetIt имеет следующие преимущества:

- *Независимость от BuildContext* — доступ к зависимостям возможен в любом месте кода, включая функции, классы моделей и сервисы
- *Упрощенная структура* — не требуется оборачивать приложение в провайдеры и создавать дополнительные виджеты
- *Тестируемость* — легко подменять зависимости в тестах благодаря явной регистрации
- *Отложенная инициализация* — с помощью `registerLazySingleton` тяжелые объекты создаются только при необходимости
- *Меньше boilerplate кода* — не нужны статические методы `of()`, расширения для BuildContext и обертки

Однако Inherited Widget остается встроенным механизмом Flutter и может быть предпочтительнее для простых случаев, когда зависимости нужны только в дереве виджетов.

=== Демонстрация работы приложения с GetIt

Работа приложения с GetIt идентична версии с Inherited Widget, так как оба подхода используют одинаковый интерфейс `HabbitsController` и одну и ту же реализацию `HabbitsContainer`.

#picture(
  path: img("habbit_list_initial"),
  caption: [Начальный экран со списком привычек],
  width: 40%,
) <getit-habbit-list-initial>

На #r(<getit-habbit-list-initial>) показан пустой экран списка привычек при первом запуске. Контроллер получается из GetIt контейнера на строке 19.

#picture(
  path: img("habbits_form_screen_filled"),
  caption: [Заполненная форма создания привычки],
  width: 40%,
) <getit-habbit-form-filled>

На #r(<getit-habbit-form-filled>) представлена форма с заполненными данными. При нажатии кнопки "Done" в AppBar вызывается метод `_submitForm`, который обращается к контроллеру через GetIt для добавления новой привычки.

#picture(
  path: img("habbits_list_filled"),
  caption: [Список с добавленной привычкой],
  width: 40%,
) <getit-habbit-list-filled>

На #r(<getit-habbit-list-filled>) видно, что привычка успешно добавлена. Благодаря `ListenableBuilder` интерфейс автоматически обновляется при вызове `notifyListeners()` в контроллере.

#picture(
  path: img("habbits_list_screen_with_progress"),
  caption: [Обновление прогресса после действий],
  width: 40%,
) <getit-habbit-list-progress>

На #r(<getit-habbit-list-progress>) показано обновление прогресса после нажатия кнопок "Ack" и "Break". Виджеты получают доступ к контроллеру через GetIt и вызывают соответствующие методы для изменения состояния.
...
На #r(<getit-habbit-stats>) представлен экран статистики. Данные конкретной привычки получаются через метод `getHabbit()` контроллера, доступного из GetIt контейнера в любой точке приложения.

#pagebreak()
= Выводы

В ходе выполнения практической работы №8 были изучены и реализованы два подхода к управлению зависимостями в Flutter приложениях: Inherited Widget и DI контейнер GetIt.

*Inherited Widget* является встроенным механизмом Flutter для передачи данных вниз по дереву виджетов. Основные характеристики:

- Требует BuildContext для доступа к данным
- Интегрирован с жизненным циклом виджетов
- Автоматически уведомляет зависимые виджеты об изменениях через `updateShouldNotify`
- Подходит для случаев, когда зависимости нужны только в UI слое

*GetIt* — это внешний DI контейнер, работающий по паттерну Service Locator. Его преимущества:

- Не требует BuildContext — доступ к зависимостям возможен из любого места
- Поддерживает различные стратегии создания объектов (singleton, lazy singleton, factory)
- Упрощает тестирование благодаря явной регистрации зависимостей
- Уменьшает количество boilerplate кода

Оба подхода были успешно интегрированы в приложение для отслеживания привычек. Реализация показала, что выбор между Inherited Widget и GetIt зависит от конкретных требований проекта: для простых случаев достаточно встроенного Inherited Widget, для сложных приложений с множеством зависимостей предпочтительнее GetIt.


Код приложения доступен в репозитории  по ссылке https://github.com/burenotti/mirea-crossplatform-2/tree/burenin/practice-8
